...
public class Main {
    public static void main(String[] args) {
        ...
        // Log
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("855796_heapsort.txt"))) {
    ...
    static class Ordenacao {
        ...
        public static void heapsort(List<Show> lista) {
            int n = lista.size();
        ...
        private static void heapify(List<Show> lista, int n, int i) {
            int maior = i;
            int esquerda = 2 * i + 1;
            int direita = 2 * i + 2;
            ...
            if (esquerda < n) {
                comparacoes++;
                if (compareShows(lista.get(esquerda), lista.get(maior)) > 0) {
                    maior = esquerda;
                }
            }
            ...
            if (direita < n) {
                comparacoes++;
                if (compareShows(lista.get(direita), lista.get(maior)) > 0) {
                    maior = direita;
                }
            }
        ...
        private static int compareShows(Show a, Show b) {
            int cmp = a.getDirector().compareToIgnoreCase(b.getDirector());
            if (cmp != 0) return cmp;
            return a.getTitle().compareToIgnoreCase(b.getTitle());
        }
        
        private static void swap(List<Show> lista, int i, int j) {
