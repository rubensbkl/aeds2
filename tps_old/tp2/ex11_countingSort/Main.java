...
public class Main {
    public static void main(String[] args) {
        ...
        // Log
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("855796_countingsort.txt"))) {
    ...
    static class Ordenacao {
        ...
        public static void countingSort(List<Show> lista) {
            comparacoes = 0;
            movimentacoes = 0;
        
            lista.sort(Comparator.comparing(Show::getTitle, String.CASE_INSENSITIVE_ORDER));

            int min = Integer.MAX_VALUE;
            int max = Integer.MIN_VALUE;
        
            for (Show s : lista) {
                int year = s.releaseYear;
                if (year < min) min = year;
                if (year > max) max = year;
            }
        
            int range = max - min + 1;
            List<List<Show>> count = new ArrayList<>(Collections.nCopies(range, null));
            for (int i = 0; i < range; i++) {
                count.set(i, new ArrayList<>());
            }
        
            for (Show s : lista) {
                count.get(s.releaseYear - min).add(s);
                movimentacoes++;
            }
        
            int index = 0;
            for (List<Show> bucket : count) {
                for (Show s : bucket) {
                    lista.set(index++, s);
                }
            }
