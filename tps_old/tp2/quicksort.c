...
int comparaDateTitle(Show a, Show b, int *comparacoes) {
    (*comparacoes)++;
    int cmp_date = cmp_insensitive(a.date_added, b.date_added);
    if (cmp_date == 0) {
        (*comparacoes)++;
...
int partition(Show *array, int low, int high, int *comparacoes, int *movimentos) {
    Show pivot = array[high];
    int i = low - 1;
    for (int j = low; j < high; j++) {
        if (comparaDateTitle(array[j], pivot, comparacoes) <= 0) {
            i++;
...
void quicksort(Show *array, int low, int high, int *comparacoes, int *movimentos) {
    if (low < high) {
        int pi = partition(array, low, high, comparacoes, movimentos);
        quicksort(array, low, pi - 1, comparacoes, movimentos);
        quicksort(array, pi + 1, high, comparacoes, movimentos);
    }
}
...
int main() {
    ...
    quicksort(selected, 0, selected_count - 1, &comparacoes, &movimentos);
    clock_t fim = clock();
    double duracao = (double)(fim - inicio) / CLOCKS_PER_SEC;

    FILE *log = fopen("844933_quicksort.txt", "w");
    if (log) {
