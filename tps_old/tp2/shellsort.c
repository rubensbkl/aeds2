...
Show* search_show_by_id(const char* id) {
    for (int i = 0; i < total_shows; i++) {
        if (cmp_insensitive(all_shows[i].show_id, id) == 0) {
            return &all_shows[i];
        }
    }
    return NULL;
...
void selecaoRecursivo(Show *array, int inicio, int tamanho, int *comparacoes, int *movimentos) {
    if (inicio >= tamanho - 1)
        return;
    ...
    for (int i = inicio + 1; i < tamanho; i++) {
        (*comparacoes)++;
        if (cmp_insensitive(array[i].title, array[menor].title) < 0) {
            menor = i;
        }
    }

    if (menor != inicio) {
        swap(&array[inicio], &array[menor]);
        (*movimentos)++;
    }

    selecaoRecursivo(array, inicio + 1, tamanho, comparacoes, movimentos);
}
...
int comparaShows(Show a, Show b, int *comparacoes) {
    (*comparacoes)++;
    int cmp_type = cmp_insensitive(a.type, b.type);
    if (cmp_type == 0) {
        (*comparacoes)++;
        return cmp_insensitive(a.title, b.title);
    }
    return cmp_type;
}

void shellsort(Show *array, int tamanho, int *comparacoes, int *movimentos) {
    for (int gap = tamanho / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < tamanho; i++) {
            Show temp = array[i];
            int j = i;
            while (j >= gap && comparaShows(array[j - gap], temp, comparacoes) > 0) {
                array[j] = array[j - gap];
                j -= gap;
                (*movimentos)++;
            }
...
int main() {
    ...
    shellsort(selected, selected_count, &comparacoes, &movimentos);
    clock_t fim = clock();
    double duracao = (double)(fim - inicio) / CLOCKS_PER_SEC;

    FILE *log = fopen("844933_shellsort.txt", "w");
    if (log) {
