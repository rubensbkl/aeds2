 ...
 *
 * TP04Q06 - Tabela Hash Direta com Rehash - v1.0 - 26 / 06 / 2025
...
class TabelaHashRehash {
    ...
    public int hash1(String titulo) {
        int hash = 0;
    ...
    public int hash2(String titulo) {
        int hash = 0;
        for (char c : titulo.toCharArray()) {
            hash += c;
        }
        return (hash + 1) % this.tamanho;
    }
    ...
    public boolean inserir(String tituloShow) {
        ...
        int pos1 = hash1(tituloShow);
        int pos2 = hash2(tituloShow);
        
        if (this.tabela[pos1] == null) {
            this.tabela[pos1] = tituloShow;
            return true;
        }
        
        if (this.tabela[pos2] == null) {
            this.tabela[pos2] = tituloShow;
            return true;
        }
    ...
    public void pesquisar(String titulo) {
        int pos1 = hash1(titulo);
        int pos2 = hash2(titulo);

        Logs.countComp(1);
        if (tabela[pos1] != null && tabela[pos1].equals(titulo)) {
            System.out.println(" (Posicao: " + pos1 + ") SIM");
            return;
        }

        Logs.countComp(1);
        if (tabela[pos2] != null && tabela[pos2].equals(titulo)) {
            System.out.println(" (Posicao: " + pos2 + ") SIM");
            return;
        }
        
        System.out.println(" (Posicao: " + pos1 + ") NAO");
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        TabelaHashRehash hashRehash = new TabelaHashRehash();
        ...
        while (!(stringEntrada = sc.nextLine()).equals("FIM")) {
            ...
            if (foundShow != null) {
                hashRehash.inserir(foundShow.getTitle());
            } else {
        ...
        while (!(stringEntrada = sc.nextLine()).equals("FIM")) {
            ...
            hashRehash.pesquisar(tituloShow);
        }

        Logs.endTimer();

        Logs.createLog("855796", "hashRehash");
