 ...
 *
 * TP06Q03 - Pilha de Registros em C - v1.0 - 03 / 11 / 2025
...
// Empilhar (push)
void empilhar(Pilha* p, Jogo* jogo) {
...
// Mostrar pilha (de baixo pra cima)
void mostrarPilhaRec(CelulaPilha* c, int idx) {
    if (c != NULL) {
        mostrarPilhaRec(c->prox, idx - 1);
        printf("[%d] ", idx);
        imprimeJogo(c->jogo);
    }
}
...
void mostrarPilha(Pilha* p) {
    mostrarPilhaRec(p->topo, p->tamanho - 1);
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (entrada[0] == 'I') {
            int id;
            sscanf(entrada, "I %d", &id);
            Jogo* jogo = buscaJogo(&biblioteca, id);
            empilhar(&pilha, jogo);
        } else if (entrada[0] == 'R') {
            Jogo* removido = desempilhar(&pilha);
            printf("(R) %s\n", removido->nome);
        }
    }
