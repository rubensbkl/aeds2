 ...
 *
 * TP05Q02 - Ordenação por Seleção em C
...
int main() {
    ...
    int idsSelecionados[1000], quantidadeIds = 0;
    char input[100];
    while (fgets(input, sizeof(input), stdin)) {
        trim(input);
        if (strcasecmp(input, "FIM") == 0) break;
        idsSelecionados[quantidadeIds++] = atoi(input);
    }

    Jogo* selecionados[1000];
    int quantidadeSelecionados = 0;
    for (int i = 0; i < quantidadeIds; i++) {
        Jogo* j = buscaJogo(&biblioteca, idsSelecionados[i]);
        if (j) selecionados[quantidadeSelecionados++] = j;
    }

    int comparacoes = 0, movimentacoes = 0;
    clock_t inicio = clock();
    selectionSort(selecionados, quantidadeSelecionados, &comparacoes, &movimentacoes);
    clock_t fim = clock();

    for (int i = 0; i < quantidadeSelecionados; i++) {
        imprimeJogo(selecionados[i]);
    }

    double tempo = ((double)(fim - inicio)) / CLOCKS_PER_SEC;
    FILE* log = fopen("855796_selecao.txt", "w");
    if (log) {
