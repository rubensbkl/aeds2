...
public class Main {
    public static void main(String[] args) {
        ...
        Ordenacao.selectionSort(selecionados);
        ...
        // Log
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("855796_selecao.txt"))) {
    ...
    static class Ordenacao {
        ...
        public static void selectionSort(List<Show> lista) {
            int n = lista.size();
            int limite = Math.min(10, n);
            for (int i = 0; i < limite; i++) {
                int min = i;
                for (int j = i + 1; j < n; j++) {
                    comparacoes++;
                    if (lista.get(j).getTitle().compareToIgnoreCase(lista.get(min).getTitle()) < 0) {
                        min = j;
                    }
                }
                if (min != i) {
